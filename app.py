...
def login_page():
    ...
    if st.button("Log In", key="Login_key"):
        ...
        if user:
            # Ensure the user dictionary contains the necessary keys
            user_data = {
                "email": user.get("email", email),  # Use input email if it's not found in the user dict
                "name": user.get("name", "User")  # Default name if not found
            }
            ...
            # Success message
            st.success(f"Login successful! Welcome back {user_data['name']}!")

            # Redirect to a new page after successful login
            st.session_state.current_page = "Bids and Gadgets"
        else:
...
st.markdown(
    ...
    .stButton > button {
        background-color: #FFA500; /* Green background color */
    ...
    background-color: #FFA500; /* Keep the same as hover */
    border: none;
    }
    .stButton > button:focus p {
    color: #FFA500;
...
st.markdown(
    ...
    div.stButton > button:hover {
        background-color: #FFA500;  /* No change on hover */
        border-color: #FFA500;      /* No change on hover */
        color: #020202; 
    }
    div.stButton > button:active {
        background-color: #FFA500;  /* No change on click */
        border-color: #FFA500;      /* No change on click */
...
.stTextInput input:focus {
    border: 2px solid #FFA500; /* Change border color to blue on focus */
    outline: none; /* Remove default outline */
    background-color: #D3D3D3; /* Slightly darker background on focus */
}
</style>
""", unsafe_allow_html=True)
...
expiry_date = datetime(2024, 11, 8, 0, 0, 0)
    ...
    gadgets = [
        ...
        },
        {
        ...
        },
         {
            ...
            "description": " Skilltech 15 inch To 43 inch Tilting Wall Mount Bracket",
            "price": 280,
            "product code": "s019",
            "category":  "Tv and Audio",
        },
